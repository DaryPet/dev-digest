...
| [planner](planner.md) | Opus | Read-only + Write (plan file only) | Produces a structured **Development Plan**, then saves it to `specs/<slug>.md` after approval |
| [implementer](implementer.md) | Sonnet | Read, Write, Edit, Bash, Grep, Glob | Implements ONE scoped slice of a Development Plan (UI or backend), in parallel, and self-verifies via tests |
...
| [architecture-reviewer](architecture-reviewer.md) | Opus | Read, Grep, Glob, Bash | Audits backend Onion layering and UI feature boundaries; returns severity-ranked findings (Critical/Major/Minor) with file:line evidence; read-only |
| [plan-verifier](plan-verifier.md) | Opus | Read, Grep, Glob, Bash | Extracts every plan obligation and audits whether each was implemented, returning a per-requirement verdict table and PASS/FAIL gate; read-only |
...
## The planner → implementer workflow

These two agents are designed to work as a pair:

1. **`planner`** turns a request into a Development Plan: scope, frozen
   interface contracts, a non-overlapping directory-ownership map, and a set of
   parallelizable tasks — each annotated with the exact skills to apply. It asks
   clarifying questions if needed, presents the plan in chat, and only writes it
   to `specs/<feature-slug>.md` **after the developer approves**.
2. The main session (orchestrator) launches several **`implementer`** agents in
   parallel, one per task, each given the plan's path and its task id.
...
- **Contracts before code.** The planner freezes shared interfaces (Zod/TS
  types, API shapes, error formats, DB schema deltas) so parallel implementers
  never invent conflicting ones — the most common parallel-agent failure mode.
...
- **Engineering insights, two levels.** The planner distills relevant insights
  from root + package `INSIGHTS.md` into the plan; each implementer additionally
  reads its own package's `INSIGHTS.md` on the spot. New insights are reported
  in the handoff and persisted by the orchestrator (not written by parallel
  implementers, to avoid worktree write conflicts).
- **Skills are baked into both.** The planner knows the full skill catalog and
  assigns skills per task; the implementer invokes the backend or UI skill set
  depending on where its scope lives.

## What the planner and implementer are based on
...
- **Spec-driven development** — separate design from implementation; the planner
  emits a machine-usable spec that the implementer consumes. *(src: 13, 14)*
- **Orchestrator-worker** decomposition — tasks emerge from analysis rather than
  a fixed pipeline. *(src: 3)*
- Development Plan section structure (objective, acceptance criteria, scope
  in/out, contracts, ownership map, tasks, tests, risks). *(src: 7, 8, 13)*
...
| 6 | https://www.pubnub.com/blog/best-practices-for-claude-code-sub-agents/ | Single-responsibility, tool hygiene, planner vs implementer scoping |
