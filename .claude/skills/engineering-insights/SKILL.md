...
Capture one durable engineering insight into the `INSIGHTS.md` of the module
the work touched, so the next session doesn't relearn it. Read what's already
...
| spans several packages | split the insight by what's relevant to each, and write to each file |

## Sections in each INSIGHTS.md

Entries live under one of these `##` headings:

- **What Works** — patterns and working approaches
- **What Doesn't Work** — antipatterns and dead-ends
- **Codebase Patterns & Tool/Library Notes** — conventions, gotchas, quirks
- **Decisions** — the choice made + the alternative rejected + why
- **Recurring Errors & Fixes**
- **Session Notes** — dated, one line per point
- **Open Questions**

## What counts (banality test)

Every entry must be actionable **cold** — a future agent reads it and knows
what to do, with no follow-up. Test before writing: *"if this would be obvious
to anyone reading the code, don't write it."*

| ❌ Noise | ✅ Useful (actionable cold) |
|---|---|
| "Promises can be tricky" | "`Promise.all()` on the ingest pipeline times out past 50 items — use `Promise.allSettled()` in batches of 10" |
...
4. **Dedup.** Drop any candidate already covered by an existing entry. If
   reality contradicts an old entry, add a new dated note that supersedes it —
...
- **Re-read the target `INSIGHTS.md` immediately before writing** — its state
...
- **Preserve verbatim** the `# <module> — insights` header, the preamble, every
  section heading, and every entry already in the file. New content is only
  ever *added*.
- **Corrections are additive** — supersede a wrong entry with a new dated note;
  do not rewrite or delete the old one.
- **Idempotent** — if an equivalent entry already exists, skip it (no
  duplicate, no rewrite).

## Maintenance (not per-session)

Append-only keeps the file growing, so keep it lean out of band: prune monthly
(drop fixed-bug, duplicate, and never-needed entries), aim for ~30 high-value
entries per file before splitting into domain files, and treat the file as a
reviewed draft — spot-check it, since an incorrect entry propagates to every
future session until corrected.
